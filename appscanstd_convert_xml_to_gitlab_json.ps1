...
ForEach ($file in $files){
  ...
  ForEach ($i in $totalIssues) {
    ...
    $issueSolution=($xml.'xml-report'.'remediation-group'.item | Where-Object {$_.id -eq $xml.'xml-report'.'issue-group'.item[0].remediation.ref}).name.Replace('"','');
