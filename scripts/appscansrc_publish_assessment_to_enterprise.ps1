...
if ($compiledArtifactFolder -ne "none"){
  ...
  write-output "pase $CI_PROJECT_DIR\$compiledArtifactFolder\$CI_PROJECT_NAME-$CI_JOB_ID.ozasmt -aseapplication $aseAppName -name $CI_PROJECT_NAME-$CI_JOB_ID" >> scriptpase.scan
